...
NodeAntrian* qHead = nullptr;
NodeAntrian* qTail = nullptr;

NodeRiwayat* sTop  = nullptr;
...
bool isQueueEmpty() { return qHead == nullptr; }
bool isStackEmpty() { return sTop  == nullptr; }
...
Hewan dequeue() {
    ...
    NodeAntrian* temp  = qHead;
    Hewan        hasil = temp->data;
    qHead = qHead->next;
...
void pushRiwayat(RiwayatTindakan* r) {
    ...
    baru->next = sTop;
    sTop       = baru;
    cout << "\n  [v] Tindakan \"" << r->tindakan
         << "\" untuk " << r->namaHewan << " dicatat.\n";
}
...
RiwayatTindakan popRiwayat() {
    ...
    NodeRiwayat*    temp  = sTop;
    RiwayatTindakan hasil = temp->data;
    sTop = sTop->next;
    delete temp;
    return hasil;
}
...
void tampilRiwayat() {
    ...
    cout << "  +----+------+--------------------+--------------------+" << endl;
    NodeRiwayat* curr = sTop;
    int no = 1;
    while (curr != nullptr) {
        RiwayatTindakan* r = &(curr->data);
        cout << "  | " << no;
        if (no < 10) cout << " ";
        cout << " | " << r->idHewan;
...
void tambahData(Hewan* arr, int &n) {
    ...
    cout << "  ID Hewan    : "; cin >> p->id; cin.ignore(1000, '\n');
    cout << "  Nama Hewan  : "; getline(cin, p->nama);  bersihInput(p->nama);
    cout << "  Jenis Hewan : "; getline(cin, p->jenis); bersihInput(p->jenis);
    cout << "  Layanan     : "; getline(cin, p->layanan); bersihInput(p->layanan);
    cout << "  Harga (Rp)  : "; cin >> p->harga; cin.ignore(1000, '\n');
    n++;
...
void linearSearch(Hewan* arr, int n, string target) {
    ...
    if (!ketemu) cout << "  [!] Nama hewan tidak ditemukan.\n";
}
...
int main() {
    ...
    do {
        ...
        if (pilihan == 1) {
            ...
            RiwayatTindakan r;
            cout << "  ID Hewan    : "; cin >> r.idHewan; cin.ignore(1000, '\n');
            cout << "  Nama Hewan  : "; getline(cin, r.namaHewan); bersihInput(r.namaHewan);
            cout << "  Tindakan    : "; getline(cin, r.tindakan);  bersihInput(r.tindakan);
            pushRiwayat(&r);
